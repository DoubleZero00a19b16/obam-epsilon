...
class AzerbaijaniAspectClustering:
    def __init__(self, model_name='paraphrase-multilingual-MiniLM-L12-v2'):
        ...
        # Define short keywords that need word boundary checking
        self.boundary_check_keywords = {
            'iy', 'iyi', 'iyir',
            'dad', 'dadı', 'dadlı', 'dadın',
            'duz', 'duzlu',
            'şor', 'şordu',
    ...
    def detect_primary_aspect(self, review):
        ...
        for aspect, data in self.problem_keywords.items():
            ...
            for keyword in data['keywords']:
                ...
                # Check if keyword needs word boundary validation
                if keyword in self.boundary_check_keywords:
                    # Use word boundary check for short keywords
                    pattern = r'\b' + re.escape(keyword) + r'\b'
                    if re.search(pattern, review_lower):
                        matched = True
                else:
                    # Regular substring match for phrases and longer words
...
if __name__ == "__main__":
    # Comprehensive sample product reviews (160+ examples) - PHYSICAL STORE ONLY
    sample_reviews = [
    ...
    print("Running Final Keyword-Based Clustering (with Word Boundary Checking)...")
    ...
    test_cases = [
        ...
        ("Miqdar az", "Az miqdar"),
        ("dar", "Ölçü problemi"),
        ("Çox dar gəldi", "Ölçü problemi"),
        ("qab", "Qablaşdırma problemi"),
        ("Qab zədəli", "Qablaşdırma problemi")
    ]
    ...
    for test_input, expected_output in test_cases:
        ...
        if test_dict['clusters']:
            ...
            print(f"✗ Input: '{test_input}' → Expected: {expected_output}, Got: No cluster")